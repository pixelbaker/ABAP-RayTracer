CLASS ucl_art_hammersley DEFINITION
  FINAL
  FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS.
...
CLASS ucl_art_hammersley IMPLEMENTATION.
  METHOD new_by_num_samples.
    "Test, that the constructor by num_samples works

    "When
    DATA(cut) = zcl_art_hammersley=>new_by_num_samples( 2 ).
  ...
  METHOD new_default.
    ...
    "When
    DATA(cut) = zcl_art_hammersley=>new_default( ).
  ...
  METHOD new_copy.
    ...
    "Given
    DATA(sampler) = zcl_art_hammersley=>new_by_num_samples( 2 ).

    "When
    DATA(cut) = zcl_art_hammersley=>new_copy( sampler ).
