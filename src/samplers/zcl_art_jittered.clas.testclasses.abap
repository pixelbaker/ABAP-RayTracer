CLASS ucl_art_jittered DEFINITION
  FINAL
  FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS.
...
CLASS ucl_art_jittered IMPLEMENTATION.
  ...
  METHOD new_by_num_samples.
    ...
    "When
    DATA(cut) = zcl_art_jittered=>new_by_num_samples( 2 ).
  ...
  METHOD new_default.
    ...
    "When
    DATA(cut) = zcl_art_jittered=>new_default( ).
  ...
  METHOD new_copy.
    ...
    "Given
    DATA(sampler) = zcl_art_jittered=>new_by_num_samples( 2 ).

    "When
    DATA(cut) = zcl_art_jittered=>new_copy( sampler ).
